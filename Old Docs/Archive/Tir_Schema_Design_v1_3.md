...
*v1.3, April 2026. Adds `chunks_fts` virtual table in working.db for BM25 lexical retrieval per the Retrieval Design v1 (hybrid vector + BM25 day-one). No other tables changed from v1.2. Previous: v1.2 added channel_identifiers, role, and documented name-embedding-in-chunks. This revision: FTS5 index.*
...
CREATE TABLE users (
    ...
    name TEXT NOT NULL,          -- Display name. "Lyle", "AgentX", etc.
...
```sql
PRAGMA journal_mode = WAL;      -- Allows concurrent reads while a write is happening.
...
More detailed than the archive version. This is where user metadata lives and where it can be updated.
...
Metadata and raw content for non-conversation material that's been ingested into ChromaDB (articles, files, URLs). The raw content is stored here as a safety net — if the chunking strategy changes, the original is available to re-chunk from.
...
CREATE TABLE documents (
    ...
    content TEXT,                                   -- Full original text. The rebuild safety net.
    created_at TEXT NOT NULL
);
```
...
ChromaDB is derived data. Everything in it is rebuildable from the working store's messages and documents tables. During normal operation, chunks are only added, never modified or deleted. During a rebuild (e.g., when chunking strategy changes during retrieval tuning), the collection is wiped and recreated from source. chunks_fts is similarly rebuildable from ChromaDB content. The "never delete" principle (Principle 6) protects the archive — derived stores are expendable by design.
...
The name is pulled from `users.name` at chunk-creation time and embedded into the chunk text. This means chunks preserve the name the person had when the conversation happened — renames or deletions in the `users` table don't alter existing chunks. The raw `messages.content` field in both archive and working store remains clean (the actual text the person typed); name embedding happens only at the chunking step.
...
1. Metadata and full original content written to working.db documents table.
...
4. Overnight: optional summary generated for UI display.
...
## Schema migration from v1.2 to v1.3

Adding `chunks_fts` is additive. Existing working.db instances need the virtual table created; no existing table or row is modified.

Migration steps:

1. Add the `CREATE VIRTUAL TABLE chunks_fts USING fts5(...)` statement to the working.db schema.
2. Update the expected-schema descriptor in bootstrap verification to include chunks_fts. Note that virtual tables don't behave the same as regular tables under `PRAGMA table_info` — the verification code may need a slight adjustment to accept virtual tables by name without column-level checks, or to use `SELECT name FROM sqlite_master WHERE type='table'` which covers both.
3. On first run after migration: chunks_fts is empty. Either (a) run a one-time backfill that iterates ChromaDB's collection and inserts every existing chunk into chunks_fts, or (b) accept that the index fills in as new chunks are written and close-time rechunks reindex old conversations. For Tír's pre-launch state (no production data yet), (b) is sufficient; add backfill if a real corpus ever needs indexing.

The BM25 Integration Spec (separate Dev_Doc) handles this in code form. This schema doc describes the target state.
...
- **Autonomous window concurrency model.** The schema supports concurrent access via WAL mode. Whether autonomous work runs as one process or many is an upstream design question.
...
*Project Tír Memory Layer Schema · v1.3 · April 2026*
