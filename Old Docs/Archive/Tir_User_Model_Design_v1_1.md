...
*Draft v1.1, April 2026. Revised from v1: user names are immutable after creation (resolves name-embedding-in-chunks consistency); users with open conversations cannot be deleted (close-time rechunking requires name lookup). All other content unchanged.*
...
This document specifies that model for v1.1.
...
CREATE TABLE users (
    ...
    name TEXT NOT NULL,              -- Display name. Immutable after creation.
...
- `name` is what the entity sees in retrieval framing. It's the display name, not a username or login. **Names are immutable after creation.** The name is embedded directly into chunk text at chunk-creation time (see "Name handling in chunks" below). If names could change, the value in the users table and the value in stored chunks would drift — she'd remember "Sarah" in her chunks but the system would now call that user "Alex." Immutability eliminates this class of inconsistency entirely.
...
1. Provide a display name. **This name is permanent — choose carefully.**
...
- Can create and delete users (subject to deletion constraints — see below).
...
**Names and channel identifiers only.** No pre-seeded relationship context, no notes, no "this is my wife."
...
- **Names are immutable, so chunks and the users table are always consistent.** There is no drift scenario — the name in the chunk is the name the user has, permanently.
...
It does **not** apply to operational/admin data — users, channel identifiers, session tokens, skill registry metadata, and similar. Those can be deleted when appropriate, subject to one constraint:

**Users with open conversations cannot be deleted.** Close-time rechunking (per Chunking Strategy v1.1) re-chunks the entire conversation from scratch, which requires looking up the user's name to embed it in the chunk text. If the user row is deleted while a conversation is still open (`ended_at IS NULL`), that lookup fails and rechunking breaks. The admin deletion path must check for open conversations and refuse deletion until they are closed. This is enforced at the admin-action layer, not via a database constraint.

Otherwise:

- **User row deletion is allowed** (once all conversations are closed). If someone leaves and Lyle wants them out of the system, the user row and their channel identifiers can be removed. They can no longer reach the entity.
...
- **Schema Design v1.4** — specifies `users` and `conversations` tables. This doc adds `channel_identifiers` and fills in the `users.role` field. Users.name is immutable in both archive and working store.
...
- **Context Construction v1.1 (retrieved memory framing)** — uses the user's `name` for conversation chunk framing. The name is embedded directly into chunk text at chunk-creation time (see "Name handling in chunks"), which resolves the "user identifier resolution timing" open question from that doc: the name is part of the stored chunk, not resolved at retrieval time.
- **Autonomous Window Design v1.1 (gateway pattern)** — each adapter resolves user identity before passing normalized messages to the engine. This doc specifies the resolution logic.
...
- **Self-service user creation.** Any flow where users create themselves. Not wanted for v1.1.
...
*Project Tír User Model Design · v1.1 · April 2026*
